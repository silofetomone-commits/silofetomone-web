...
const firebaseConfig = {
  apiKey: "AIzaSyDFBNpi-yEYiOcd0qFQnjABE0oKxJ4twk0",
  authDomain: "db-silofetomone.firebaseapp.com",
  projectId: "db-silofetomone",
  storageBucket: "db-silofetomone.firebasestorage.app",
  messagingSenderId: "910711011058",
  appId: "1:910711011058:web:e6a1731f6a05d42352026b",
  measurementId: "G-RY2XJ5JG19"
};
...
// --- CONSTANTS ---
const DEFAULT_PENATUA = { '1': "Penatua Veni Isabela Nokas", '2': "Penatua Deni Toto", '3': "Penatua Hesti Tungga", '4': "Penatua Welhelmina Bonlai" };
const DEFAULT_PROFIL = { sinode: "Gereja Masehi Injili di Timor (GMIT)", klasis: "Nama Klasis", jemaat: "Silofetomone", mataJemaat: "-", alamat: "Alamat Gereja" };
...
const MAJELIS_HEADER_MAP = { "Nomor Rayon": "noRayon", "Nama Lengkap": "namaLengkap", "Nama Panggilan": "namaPanggilan", "Tempat Lahir": "tanggalLahir", "Jenis Kelamin": "jk", "Status Menikah": "statusMenikah", "Pekerjaan": "pekerjaan", "Jabatan di Masyarakat": "jabatanMasyarakat", "Jabatan Pelayanan": "jabatanPelayanan", "Jumlah Saudara": "jumlahSaudara", "Anak Ke": "anakKe", "Golongan Darah": "goldar", "Gereja Baptis": "gerejaBaptis", "Tanggal Baptis": "tanggalBaptis", "Gereja Sidi": "gerejaSidi", "Tanggal Sidi": "tanggalSidi", "Gereja Nikah": "gerejaNikah", "Tanggal Nikah": "tanggalNikah", "Nama Ayah": "namaAyah", "Nama Ibu": "namaIbu", "Nama Suami/Istri": "namaPasangan", "Tempat Lahir Pasangan": "tempatLahirPasangan", "Tanggal Lahir Pasangan": "tanggalLahirPasangan", "Pekerjaan Pasangan": "pekerjaanPasangan", "Gereja Baptis Pasangan": "gerejaBaptisPasangan", "Tanggal Baptis Pasangan": "tanggalBaptisPasangan", "Gereja Sidi Pasangan": "gerejaSidi Pasangan", "Tanggal Sidi Pasangan": "tanggalSidiPasangan", "Nama SD": "namaSD", "Tahun Tamat SD": "tahunSD", "Nama SMP": "namaSMP", "Tahun Tamat SMP": "tahunSMP", "Nama SMA": "namaSMA", "Tahun Tamat SMA": "tahunSMA", "Nama PT": "namaPT", "Tahun Lulus PT": "tahunPT", "Pernah melayani di Gereja?": "pernahMelayani", "Jabatan Lama": "jabatanLama", "Periode Pelayanan": "periodePelayanan" };

const FORM_MAJELIS = [
  {t: 'Profil Dasar', f: [
    {l:'Nomor Rayon', k:'noRayon', t:'sel', opts:['1','2','3','4']}, {l:'Nama Lengkap', k:'namaLengkap', req:true}, {l:'Nama Panggilan', k:'namaPanggilan'}, {l:'Tempat Lahir', k:'tempatLahir'}, {l:'Tanggal Lahir', k:'tanggalLahir', t:'date'}, {l:'Jenis Kelamin', k:'jk', t:'sel', opts:['Laki-laki', 'Perempuan']},
...
// --- UTILS ---
const pad0 = n => (n && String(n).trim().length===1) ? '0'+String(n).trim() : String(n||'').trim();
...
const isMatchKat = (d, kat) => {
  ...
  const a = calculateAge(d.tanggalLahir); const l = isL(d.jk);
  const p = isP(d.jk);
  switch(kat) {
...
function InfografisTab({ data, filterRayon, type }) {
  ...
  const nkA = dAktif.filter(d=>(d.jenisNikah||[]).includes('Nikah Adat')).length;
  const nkG = dAktif.filter(d=>(d.jenisNikah||[]).includes('Nikah Gereja/Masehi')).length;
  const nkS = dAktif.filter(d=>(d.jenisNikah||[]).includes('Nikah Catatan Sipil/BS')).length;
  ...
  return (
    <div className="w-full bg-white text-black p-4 text-[11px] font-sans">
       ...
       <div className="flex justify-between items-center mb-4 border-b-2 border-black pb-2">
          <div>
              <h1 className="text-xl font-bold uppercase tracking-wider">{profilGereja?.sinode || 'GEREJA MASEHI INJILI DI TIMOR (GMIT)'}</h1>
              <h2 className="text-lg font-bold uppercase">DATA JEMAAT {profilGereja?.jemaat || ''}</h2>
          </div>
       ...
       <div className="grid grid-cols-3 gap-2 mb-4 text-xs">
          ...
          <div>
            <div className="flex justify-end"><span className="w-24 font-bold text-right">No. KK</span><span>: {safeStr(dataKk.idKk)}</span></div>
          </div>
       </div>
       
       <h3 className="font-bold text-sm mb-1 uppercase bg-gray-200 p-1 text-center border border-black">Tabel 1: Identitas Umum</h3>
       <table className="w-full border-collapse border border-black mb-4 text-[10px] text-center">
       ...
       <div className="flex justify-between mt-8 px-16 text-sm">
          ...
          <div className="text-center">
             ...
             <p className="mb-16">Majelis Jemaat - Penatua Rayon {safeStr(dataKk.noRayon)},</p>
...
function PrintMajelisTemplate({ majelisToPrint, majelisData, penatuaMap, profilGereja, onBack }) {
  ...
  return (
    <div className="w-full bg-white text-black p-8 text-sm font-sans max-w-4xl mx-auto border shadow-lg print:border-none print:shadow-none print:m-0 print:p-0">
       ...
       <div className="text-center mb-6 border-b-4 border-double border-black pb-4"><img src="https://i.imgur.com/XV3hpOH.png" alt="Logo" className="w-20 h-20 mx-auto mb-2" /><h1 className="text-2xl font-black uppercase">PROFIL PELAYAN / MAJELIS JEMAAT</h1><h2 className="text-lg font-bold uppercase">{profilGereja?.jemaat || ''} - RAYON {safeStr(mj.noRayon)}</h2></div>
       ...
       <div className="mt-10 text-right pr-8 text-sm">
          <p className="mb-16 uppercase">{profilGereja?.jemaat || ''}, {getFormatDate()}</p>
          <p className="font-bold underline uppercase">{safeStr(mj.namaLengkap)}</p>
          <p>Pelayan / Majelis</p>
       </div>
       <div className="flex items-center justify-center mt-8 gap-4 no-print">
  ...
  return (
    <div className="block w-full bg-white text-black p-8 text-sm font-sans max-w-5xl mx-auto border shadow-lg print:border-none print:shadow-none print:m-0 print:p-0">
       ...
       <div className="text-center mb-6 border-b-4 border-double border-black pb-4">
          ...
          <h2 className="text-lg font-bold uppercase">{profilGereja?.jemaat || ''}</h2>
       ...
       <div className="flex justify-end mt-10 pr-8 text-sm"><div className="text-center"><p className="mb-16 uppercase">{profilGereja?.jemaat || ''}, {getFormatDate()}</p><p className="font-bold underline uppercase">Majelis Jemaat</p></div></div>
       <div className="flex items-center justify-center mt-8 gap-4 no-print"><button onClick={() => window.print()} className="px-4 py-2 bg-blue-600 font-bold text-white rounded shadow">🖨️ Tekan Untuk Cetak Kertas Ini</button><button onClick={onBack} className="px-4 py-2 bg-gray-500 font-bold text-white rounded shadow"><ArrowLeft className="w-4 h-4 inline mr-2"/> Kembali</button></div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-gray-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 font-medium text-center uppercase">{profilGereja?.jemaat || 'SILOFETOMONE'}</p>
        </div>
...
export default function App() {
  ...
  const [profilGereja, setProfilGereja] = useState(DEFAULT_PROFIL);
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    const u1 = onSnapshot(collection(db, 'jemaat'), (s) => { setJemaatData(s.docs.map(d => ({ dbId: d.id, ...d.data() }))); setIsLoading(false); });
  ...
  const recordHistory = async (action, col, target) => {
    try { await addDoc(collection(db, 'history'), { action, collection: col, target: target || 'Data', user: appUser?.name || appUser?.role || 'System', timestamp: Date.now() });
    } catch(e) {}
  };

  const handleRestoreData = (row) => {
    setConfirmDialog({
      isOpen: true,
      title: "Tarik Data",
      ...
      onConfirm: async () => {
        ...
        try { await updateDoc(doc(db, 'jemaat', row.dbId), { statusKeanggotaan: 'Aktif', statusHidup: 'Hidup', tanggalKematian: '', tanggalPenguburan: '', pindahKeJemaat: '', tanggalPindah: '' }); recordHistory('RESTORE', 'jemaat', row.namaLengkap); } catch (e) {}
      }
    });
  };
  ...
  const handleCleanAll = (colName) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        let successCount = 0;
        for (const d of dataToDel) { try { await deleteDoc(doc(db, colName, d.dbId)); successCount++; } catch(e){} }
  ...
  const handleExportSinode = () => {
    ...
    l.download = `Data_Laporan_Sinode_${(profilGereja?.jemaat || 'SILOFETOMONE').replace(/\s+/g, '_')}.xls`; 
  ...
  const handleImportCSV = async (e) => {
    ...
    reader.onload = async (event) => {
      ...
      const existingIdentities = new Set(targetData.map(d => (d.namaLengkap||'').toLowerCase().trim()));
      ...
      for (let i=1; i<rows.length; i++) {
        ...
        headers.forEach((h, idx) => {
           ...
           if(hLow.includes('tgl') || hLow.includes('tanggal') || hLow.includes('lahir') || hLow.includes('baptis') || hLow.includes('sidi') || hLow.includes('nikah') || hLow.includes('masuk')) {
           ...
           if(isMajelis) {
               if(MAJELIS_HEADER_MAP[h]) { 
                  ...
                  if(m && (parseInt(m[2])-1)>=0 && (parseInt(m[2])-1)<6) {
                     const fk=m[1], ai=parseInt(m[2])-1;
                     if(fk==='Nama') ank[ai].nama=v; if(fk==='Tempat Lahir') ank[ai].tempatLahir=v; if(fk.includes('Lahir')) ank[ai].tanggalLahir=v;
                     if(fk==='Gereja Baptis') ank[ai].gerejaBaptis=v; if(fk.includes('Baptis')) ank[ai].tanggalBaptis=v;
                     if(fk==='Gereja Sidi') ank[ai].gerejaSidi=v; if(fk.includes('Sidi')) ank[ai].tanggalSidi=v;
                  }
               }
           } else {
               let k = JEMAAT_HEADER_MAP[h] || h; 
               k = String(k).replace(/[~*/\[\]]/g, '').trim(); 
               if(k==='jenisNikah' && v) d[k] = v.split(',').map(s=>s.trim()); else d[k] = v;
           }
        });
        ...
        const idKey = (d.namaLengkap || '').toLowerCase().trim();
        if(existingIdentities.has(idKey)) { duplicateCount++; continue; }

        if(!isMajelis) { d.statusKeanggotaan = 'Aktif'; d.statusHidup = 'Hidup'; }
        
        try { await addDoc(collection(db, typeName), d); successCount++; existingIdentities.add(idKey); } catch(e) {}
      }
      
      recordHistory('IMPORT', typeName, `${successCount} Data CSV Masuk`);
      showAlert('Selesai!', `Berhasil masuk: ${successCount} data.\nDitolak (Dobel Nama Lengkap): ${duplicateCount} data.`); 
      setIsLoading(false);
    };
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => setFormData(prev => ({ ...prev, fotoBase64: reader.result }));
  ...
  const saveDocument = async (collectionName, data, id = null) => {
    try {
      ...
      Object.entries(data).forEach(([key, val]) => {
         if (val !== undefined && key !== 'dbId') {
             const safeKey = String(key).replace(/[~*/\[\]]/g, '').trim();
             cleanData[safeKey] = val;
         }
      });
      if (id) { 
          await updateDoc(doc(db, collectionName, id), cleanData);
          recordHistory('EDIT', collectionName, cleanData.namaLengkap || cleanData.kepalaKeluarga || id); 
      } else { 
          await addDoc(collection(db, collectionName), cleanData);
          recordHistory('TAMBAH', collectionName, cleanData.namaLengkap || cleanData.kepalaKeluarga || 'Data Baru'); 
      }
    } catch (error) { 
  ...
  const requestDelete = (collectionName, id, name) => {
    setConfirmDialog({
      isOpen: true,
      ...
      onConfirm: async () => {
        setConfirmDialog({ isOpen: false });
        try { await deleteDoc(doc(db, collectionName, id)); recordHistory('HAPUS', collectionName, name || id); showAlert("Sukses", "Data berhasil dihapus!"); } catch (error) { showAlert("Error", "Gagal menghapus."); }
      }
    });
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    else if (modalMode === 'addMajelis' || modalMode === 'editMajelis') {
      await saveDocument('majelis', dataToSave, docId);
    }
  ...
  const handleCheckboxChange = (name, opt, isChecked, currentArr) => {
    handleFormChange({ target: { name, value: isChecked ? [...currentArr, opt] : currentArr.filter(x => x !== opt) } });
  };
  ...
  const getTabHeaders = () => {
    ...
    if (activeTab === 'Data KK') return [
      ...
      {l:'Rayon',k:'noRayon', fmt:r}, {l:'Urutan KK',k:'urutanKk'}
    ];
    if (activeTab === 'Data Jemaat') return [
      {l:'Nama Lengkap',k:'namaLengkap', fmt:v=><span className="font-bold text-blue-900">{v}</span>},
      {l:'Rayon',k:'noRayon', fmt:r}, {l:'Kepala Keluarga',k:'kepalaKeluarga', fmt:bld},
    ...
    if (activeTab === 'Status Jemaat') {
      ...
      if (activeSubTabStatus === 'Pindah Jemaat') return [{l:'Nama Lengkap',k:'namaLengkap', fmt:v=><span className="font-black text-orange-700">{v} <span className="text-gray-500 text-xs italic">(Pindah Keluar)</span></span>},{l:'L/P',k:'jk', fmt:lp},{l:'Gereja Tujuan',k:'pindahKeJemaat'},{l:'Tanggal Pindah',k:'tanggalPindah', fmt:toDisplayDate},{l:'Rayon',k:'noRayon', fmt:r}];
      if (activeSubTabStatus === 'Pindah Masuk Jemaat') return [{l:'Nama Lengkap',k:'namaLengkap', fmt:v=><span className="font-black text-teal-700">{v} <span className="text-gray-500 text-xs italic">(Pindah Masuk)</span></span>},{l:'L/P',k:'jk', fmt:lp},{l:'Asal Gereja',k:'asalJemaat'},{l:'Tanggal Masuk',k:'tanggalMasuk', fmt:toDisplayDate},{l:'Rayon',k:'noRayon', fmt:r}];
    ...
    if (activeTab === 'Profil Majelis') return [
      ...
      {l:'Nama Lengkap', k:'namaLengkap', fmt:bld}, {l:'Rayon', k:'noRayon', fmt:r},
      {l:'Jabatan', k:'jabatanPelayanan', fmt:v=><span className="text-purple-700 font-black">{v}</span>},
      {l:'L/P', k:'jk', fmt:lp}, {l:'Pekerjaan', k:'pekerjaan'}
    ];
    if (activeTab === 'Riwayat Sistem') return [
      {l:'Waktu',k:'timestamp', fmt:v=>new Date(v).toLocaleString('id-ID')},{l:'User',k:'user', fmt:bld},
      {l:'Aksi',k:'action', fmt:v=><span className={`px-2 py-1 rounded text-xs font-bold text-white ${v==='TAMBAH'?'bg-green-500':v==='EDIT'?'bg-blue-500':v==='RESTORE'?'bg-indigo-500':v==='HAPUS SEMUA'?'bg-red-700':v==='IMPORT'?'bg-amber-500':'bg-red-500'}`}>{v}</span>},
      {l:'Data Target',k:'target', fmt:(v,row)=>`${v} (${row.collection})`}
    ];
    return [];
  };
  ...
  const filteredData = useMemo(() => {
    let d = [];
    const isActive = x => x.statusKeanggotaan !== 'Meninggal' && x.statusKeanggotaan !== 'Pindah' && x.statusHidup !== 'Meninggal';
    ...
    if (activeTab === 'Data KK') d = jemaatData.filter(x => x.statusKeluarga === 'Kepala Keluarga' && isActive(x));
      ...
      else if (activeSubTabStatus === 'Pindah Masuk Jemaat') d = jemaatData.filter(isActive).filter(x => x.asalJemaat && x.asalJemaat.trim() !== '');
      ...
      else if (activeSubTabStatus === 'Ulang Tahun') {
        d = jemaatData.filter(isActive).filter(x => getMonthFromDate(x.tanggalLahir) === parseInt(filterBulan));
      }
    } 
    else if (activeTab === 'Riwayat Sistem') {
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl mt-20 mb-10 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar bg-white">
              <form id="crud-form" onSubmit={handleFormSubmit}>
                ...
                {(modalMode === 'addAnggota' || modalMode === 'addJemaat' || modalMode === 'editJemaat') && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-2">
                    ...
                    {JEMAAT_FIELDS_PRIBADI.map(f => {
                       let options = f.opts;
                       if (f.name === 'noAnggota') options = noAnggotaOpts;
                ...
                {/* --- FORM KEMATIAN / PINDAH JEMAAT / PINDAH MASUK --- */}
                {(modalMode === 'addKematian' || modalMode === 'addPindah' || modalMode === 'addPindahMasuk') && (
                  <div className="space-y-4 p-4">
                    ...
                    {modalMode === 'addPindah' && ( <><FormInput req label="Gereja Tujuan" name="pindahKeJemaat" value={formData.pindahKeJemaat} onChange={handleFormChange} /><FormInput req type="date" label="Tanggal Pindah Keluar" name="tanggalPindah" value={formData.tanggalPindah} onChange={handleFormChange} /></> )}
                    {modalMode === 'addKematian' && ( <><FormInput req type="date" label="Tanggal Kematian" name="tanggalKematian" value={formData.tanggalKematian} onChange={handleFormChange} /><FormInput req type="date" label="Tanggal Penguburan" name="tanggalPenguburan" value={formData.tanggalPenguburan} onChange={handleFormChange} /></> )}
                    {modalMode === 'addPindahMasuk' && ( <><FormInput req label="Asal Gereja / Jemaat" name="asalJemaat" value={formData.asalJemaat} onChange={handleFormChange} /><FormInput req type="date" label="Tanggal Masuk" name="tanggalMasuk" value={formData.tanggalMasuk} onChange={handleFormChange} /></> )}
                  </div>
                )}

                {/* --- FORM VIEW DETAIL JEMAAT --- */}
                {modalMode === 'viewJemaat' && (
                  <div className="p-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                     {Object.keys(JEMAAT_HEADER_MAP).filter(h => !['ID KK', 'ID Jemaat', 'No Anggota', 'Urutan KK', 'No Rayon'].includes(h)).map(header => {
                         const key = JEMAAT_HEADER_MAP[header];
        ...
        <header className="fixed top-4 left-4 right-4 bg-white/95 backdrop-blur-md rounded-3xl shadow-sm z-40 px-6 py-4 border border-gray-200 flex items-center justify-between print:hidden">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-blue-600 font-bold hidden md:flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div> {profilGereja?.jemaat || 'SILOFETOMONE'} • Akses: <span className="uppercase">{appUser?.role}</span></div>
            </div>
          </div>
            ...
            <div className="mb-4 print:hidden">
               ...
               <div className="relative inline-block w-full sm:w-auto">
                 <select value={activeSubTabStatus} onChange={(e) => setActiveSubTabStatus(e.target.value)} className={`w-full sm:w-72 appearance-none bg-white border-2 text-sm font-bold py-3 pl-4 pr-10 rounded-xl focus:outline-none focus:ring-2 cursor-pointer shadow-sm transition-all ${activeSubTabStatus === 'Ulang Tahun' ? 'border-pink-200 text-pink-700 focus:ring-pink-500' : 'border-indigo-200 text-indigo-700 focus:ring-indigo-500'}`}>
                    {['Pelayanan Kategori', 'Anggota Baptis', 'Anggota Sidi', 'Anggota Nikah', 'Pindah Jemaat', 'Pindah Masuk Jemaat', 'Data Kematian', 'Ulang Tahun'].map(sub => (
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden print:border-none print:shadow-none">
              ...
              {activeTab === 'Manajemen Data & Pengaturan' ? (
                <div className="p-8">
                   <div className="mb-8">
                      <h2 className="text-2xl font-black text-gray-800 flex items-center gap-2 mb-4"><Settings className="w-6 h-6 text-gray-600"/> Profil Gereja</h2>
                      <p className="text-gray-500 mb-4">Pengaturan ini akan diterapkan pada kop surat, cetakan dokumen, dan antarmuka aplikasi.</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-6 rounded-2xl border border-gray-200">
                         <div><label className="text-sm font-bold text-gray-700 mb-1 block">Sinode</label><input type="text" className="w-full border-2 border-gray-300 p-2.5 rounded-xl focus:border-blue-500 outline-none text-sm" value={profilGereja.sinode||''} onChange={e => setProfilGereja({...profilGereja, sinode: e.target.value})} /></div>
                         <div><label className="text-sm font-bold text-gray-700 mb-1 block">Klasis</label><input type="text" className="w-full border-2 border-gray-300 p-2.5 rounded-xl focus:border-blue-500 outline-none text-sm" value={profilGereja.klasis||''} onChange={e => setProfilGereja({...profilGereja, klasis: e.target.value})} /></div>
                         <div><label className="text-sm font-bold text-gray-700 mb-1 block">Jemaat</label><input type="text" className="w-full border-2 border-gray-300 p-2.5 rounded-xl focus:border-blue-500 outline-none text-sm" value={profilGereja.jemaat||''} onChange={e => setProfilGereja({...profilGereja, jemaat: e.target.value})} /></div>
                         <div><label className="text-sm font-bold text-gray-700 mb-1 block">Mata Jemaat</label><input type="text" className="w-full border-2 border-gray-300 p-2.5 rounded-xl focus:border-blue-500 outline-none text-sm" value={profilGereja.mataJemaat||''} onChange={e => setProfilGereja({...profilGereja, mataJemaat: e.target.value})} /></div>
                         <div className="md:col-span-2"><label className="text-sm font-bold text-gray-700 mb-1 block">Alamat Gereja</label><input type="text" className="w-full border-2 border-gray-300 p-2.5 rounded-xl focus:border-blue-500 outline-none text-sm" value={profilGereja.alamat||''} onChange={e => setProfilGereja({...profilGereja, alamat: e.target.value})} /></div>
                      </div>
                      <div className="mt-4 flex justify-end">
                         <button onClick={async () => { try { await setDoc(doc(db, 'settings', 'profil_gereja'), profilGereja, {merge:true}); showAlert("Sukses", "Profil Gereja berhasil disimpan!"); } catch(e){ showAlert("Error", "Gagal menyimpan."); } }} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-xl shadow transition-all active:scale-95">Simpan Profil</button>
                      </div>
                   </div>
                   ...
                   <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                      <div><h2 className="text-2xl font-black text-gray-800 flex items-center gap-2"><Users className="w-6 h-6 text-gray-600"/> Manajemen Nama Penatua</h2><p className="text-gray-500 mt-2">Ubah nama penatua dan atur password login khusus untuk masing-masing Rayon.</p></div>
                ...
                <>
                  <div className="p-6 border-b border-gray-100 flex flex-col xl:flex-row xl:items-center justify-between gap-4 print:hidden">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {/* OPSI LAIN DATA JEMAAT HANYA UNTUK ADMIN */}
                      ...
                      {/* OPSI LAIN MAJELIS HANYA UNTUK ADMIN */}
                      {activeTab === 'Profil Majelis' && subTabMajelis !== 'Infografis' && appUser?.role === 'admin' && (
                        <div className="flex items-center gap-2 relative">
                          ...
                          <div className="relative">
                            ...
                            {showMenuOps && ( <><div className="fixed inset-0 z-40" onClick={() => setShowMenuOps(false)}></div><div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-2xl shadow-xl z-50 overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2 duration-200"><button onClick={() => { setShowMenuOps(false); handleExportCSV(); }} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-left text-sm font-bold text-gray-700"><Download className="w-4 h-4 text-emerald-600"/> Download Excel</button><button onClick={() => { setShowMenuOps(false); fileInputRef.current.click(); }} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-left text-sm font-bold text-gray-700"><FileUp className="w-4 h-4 text-amber-500"/> Import CSV</button><div className="h-px bg-gray-100 w-full"></div><button onClick={() => { setShowMenuOps(false); handleCleanAll('majelis'); }} className="flex items-center gap-3 px-4 py-3 hover:bg-red-50 text-left text-sm font-bold text-red-600"><Trash2 className="w-4 h-4"/> Kosongkan Data</button></div></> )}
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === 'Status Jemaat' && (appUser?.role === 'admin' || appUser?.role === 'penatua') && activeSubTabStatus === 'Pindah Jemaat' && <button onClick={() => { setFormData({}); setModalMode('addPindah'); }} className="flex items-center gap-1 bg-orange-600 hover:bg-orange-700 text-white px-4 py-2.5 rounded-xl text-sm font-bold shadow-md active:scale-95"><Plus className="w-4 h-4"/> Lapor Pindah Keluar</button>}
                  ...
                  <div className="p-0">
                    <div className="hidden print:block mb-4 border-b-2 border-black pb-4">
                       ...
                       <p className="text-center font-medium text-sm">{profilGereja?.jemaat?.toUpperCase() || ''}</p>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-max">
                          ...
                          <tbody className="text-sm">
                             {currentData.length === 0 ? <tr><td colSpan="20" className="px-4 py-12 text-center text-gray-400 font-bold"><Users className="w-12 h-12 mx-auto mb-3 opacity-50"/> Tidak ada data ditemukan.</td></tr> : currentData.map((row, idx) => {
                                const startIndex = itemsPerPage === 'Semua' ? 0 : (currentPage - 1) * itemsPerPage;
                                const isEditable = canEdit(row);
                                return (
                                   <tr key={row.dbId||idx} className="hover:bg-blue-50/50 transition-colors border-b border-gray-100">
                                            ...
                                            {activeTab === 'Status Jemaat' ? (
                                               <div className="flex flex-col gap-1 w-full">
                                                 {['Data Kematian', 'Pindah Jemaat', 'Pindah Masuk Jemaat'].includes(activeSubTabStatus) && (
                                                     <>
                                                        <button onClick={() => { setFormData(row); setModalMode('viewJemaat'); }} className="p-1.5 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg transition flex items-center justify-center gap-1 w-full text-xs font-bold" title="Lihat Detail"><Eye className="w-3 h-3" /> Detail</button>
                                                        {/* Tarik data / batalkan kematian atau pindah, hanya untuk Admin */}
                                                        {appUser?.role === 'admin' && activeSubTabStatus !== 'Pindah Masuk Jemaat' && <button onClick={() => handleRestoreData(row)} className="p-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-lg transition flex items-center justify-center gap-1 w-full text-xs font-bold"><RefreshCw className="w-3 h-3" /> Tarik</button>}
                                                     </>
                                                 )}
                                               </div>
                                            ) : isEditable ? (
                                              <>
                                                ...
                                                {activeTab === 'Data KK' && (
                                                  <>
                                                    ...
                                                    <button onClick={() => {setPrintId(row.idKk); setPrintMode('kk');}} className="p-2 ml-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white rounded-lg font-black text-xs flex items-center transition" title="Cetak"><Printer className="w-4 h-4"/></button>
                                                  </>
                                                )}
                                                {activeTab === 'Profil Majelis' && <button onClick={() => {setPrintId(row.dbId); setPrintMode('majelis');}} className="p-2 ml-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white rounded-lg font-black text-xs flex items-center transition" title="Cetak"><Printer className="w-4 h-4"/></button>}
                                              </>
                                            ) : (
                                              ...
                                              (activeTab === 'Data KK' ? <button onClick={() => {setPrintId(row.idKk); setPrintMode('kk');}} className="p-1.5 ml-1 bg-indigo-100 text-indigo-700 hover:bg-indigo-600 hover:text-white rounded-lg font-black text-xs flex items-center transition"><Printer className="w-3 h-3 inline mr-1"/> Cetak KK</button>
                                              : activeTab === 'Profil Majelis' ? <button onClick={() => {setPrintId(row.dbId); setPrintMode('majelis');}} className="p-1.5 ml-1 bg-indigo-100 text-indigo-700 hover:bg-indigo-600 hover:text-white rounded-lg font-black text-xs flex items-center transition"><Printer className="w-3 h-3 inline mr-1"/> Cetak Profil</button>
