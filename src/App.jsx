...
// --- FIREBASE PRIBADI GEREJA SILOFETOMONE ---
...
// PENAMBAHAN KOLOM ASAL JEMAAT DAN TANGGAL MASUK UNTUK FITUR PINDAH MASUK JEMAAT
...
const res = []; let row = [], cell = '', inQ = false;
...
GEREJA MASEHI INJILI DI TIMOR (GMIT)DATA JEMAAT GMIT - SILOFETOMONE




Nama Kepala Keluarga: {safeStr(dataKk.kepalaKeluarga)}
Nomor HP: {safeStr(dataKk.noHp)}
Bentuk Rumah: {safeStr(dataKk.bentukRumah)}
Status Rumah: {safeStr(dataKk.statusRumah)}

Alamat Rayon: Rayon {safeStr(dataKk.noRayon)}Alamat Domisili: {safeStr(dataKk.alamat)}
No. KK: {safeStr(dataKk.idKk)}

Tabel 1: Identitas Umum

NONAMA LENGKAPNIKJKTEMPAT LAHIRTGL LAHIRDOMISILIPENDIDIKANPEKERJAANPENGHASILANGOL DARAHASURANSI KESDISABILITAS

{anggotaKk.map((ang, i) => (

{safeStr(ang.noAnggota || i+1)}<td className={text-left font-bold uppercase whitespace-nowrap ${b}}>{safeStr(ang.namaLengkap)}{safeStr(ang.nik)}
{isL(ang.jk) ? 'L' : (isP(ang.jk) ? 'P' : '-')}{safeStr(ang.tempatLahir)}{toDisplayDate(ang.tanggalLahir)}
{safeStr(ang.alamat)}{safeStr(ang.pendidikan)}{safeStr(ang.pekerjaan)}
<td className={whitespace-nowrap ${b}}>{safeStr(ang.penghasilan)}{safeStr(ang.goldar)}{safeStr(ang.asuransi)}
{ang.disabilitas === 'Ya' ? safeStr(ang.jenisDisabilitas) : '-'}

))}


Tabel 2: Riwayat Agama & Keluarga

NOTEMPAT BAPTISTGL BAPTISPENDETA BAPTISTEMPAT SIDITGL SIDIPENDETA SIDITEMPAT NIKAHTGL NIKAHPENDETA NIKAHJENIS NIKAHSTATUS KELUARGAJANDA/YATIMJABATAN JEMAAT

{anggotaKk.map((ang, i) => (

{safeStr(ang.noAnggota || i+1)}{safeStr(ang.gerejaBaptis)}{toDisplayDate(ang.tanggalBaptis)}{safeStr(ang.pendetaBaptis)}
{safeStr(ang.gerejaSidi)}{toDisplayDate(ang.tanggalSidi)}{safeStr(ang.pendetaSidi)}
{safeStr(ang.gerejaNikah)}{toDisplayDate(ang.tanggalNikah)}{safeStr(ang.pendetaNikah)}
{Array.isArray(ang.jenisNikah) ? ang.jenisNikah.join(', ') : safeStr(ang.jenisNikah)}{safeStr(ang.statusKeluarga)}{(ang.jandaDuda && ang.jandaDuda !== 'Tidak' && ang.jandaDuda !== '') ? safeStr(ang.jandaDuda) : ((ang.yatimPiatu && ang.yatimPiatu !== 'Tidak') ? safeStr(ang.yatimPiatu) : '-')}
{safeStr(ang.jabatanJemaat)}
...
PROFIL PELAYAN / MAJELIS JEMAATSILOFETOMONE - RAYON {safeStr(mj.noRayon)}



{mj.fotoBase64 ?  : PAS FOTO3x4}
...
Nama Lengkap: {safeStr(mj.namaLengkap)}
Nama Panggilan: {safeStr(mj.namaPanggilan)}
Jabatan Pelayanan: {safeStr(mj.jabatanPelayanan)}
Tempat, Tgl Lahir: {safeStr(mj.tempatLahir)}, {toDisplayDate(mj.tanggalLahir)}
Jenis Kelamin: {safeStr(mj.jk)}
Golongan Darah: {safeStr(mj.goldar)}
Anak Ke / Jml Sdr: {safeStr(mj.anakKe)} dari {safeStr(mj.jumlahSaudara)} bersaudara
Pekerjaan: {safeStr(mj.pekerjaan)}





B. RIWAYAT GEREJAWI PRIBADI


Gereja & Tgl Baptis: {safeStr(mj.gerejaBaptis)} ({toDisplayDate(mj.tanggalBaptis)})
Gereja & Tgl Sidi: {safeStr(mj.gerejaSidi)} ({toDisplayDate(mj.tanggalSidi)})
Nama Ayah / Ibu Kandung: {safeStr(mj.namaAyah)} / {safeStr(mj.namaIbu)}
...
Status Menikah: {Array.isArray(mj.statusMenikah)?mj.statusMenikah.join(', '):safeStr(mj.statusMenikah)}
Nama Suami/Istri: {safeStr(mj.namaPasangan)}
Tempat, Tgl Lahir Pasangan: {safeStr(mj.tempatLahirPasangan)}, {toDisplayDate(mj.tanggalLahirPasangan)}


Data Anak:

NoNama AnakTempat, Tgl LahirGereja BaptisGereja Sidi

{anakArr.length > 0 ? anakArr.map((a, i) => (
{i+1}<td className={text-left font-bold ${b}}>{safeStr(a.nama)}{safeStr(a.tempatLahir)}, {toDisplayDate(a.tanggalLahir)}{safeStr(a.gerejaBaptis)} {a.tanggalBaptis ? (${toDisplayDate(a.tanggalBaptis)}) : ''}{safeStr(a.gerejaSidi)} {a.tanggalSidi ? (${toDisplayDate(a.tanggalSidi)}) : ''}
)) : Tidak ada data anak tercatat.}




   <div className="mt-10 text-right pr-8 text-sm">
      <p className="mb-16">SILOFETOMONE, {getFormatDate()}</p>
...
LAPORAN DATA {safeStr(listToPrint).toUpperCase()}
SILOFETOMONE
Filter: Rayon {safeStr(filterRayon)} {listToPrint === 'Pelayanan Kategori' ? | Kategori: ${safeStr(filterKategori)} : ''}




NO
{tabCols.map(c => {c.l.toUpperCase()})}
...
{i+1}
{tabCols.map((c, j) => {c.fmt ? c.fmt(row[c.k], row) : safeStr(row[c.k])})}

))}
{(!filteredData || filteredData.length === 0) && (Tidak ada data.)}


SILOFETOMONE, {getFormatDate()}Majelis Jemaat
<button onClick={() => window.print()} className="px-4 py-2 bg-blue-600 font-bold text-white rounded shadow">🖨️ Tekan Untuk Cetak Kertas Ini Kembali
...
Masuk Sebagai<select value={role} onChange={(e) => { setRole(e.target.value); setPassword(''); setErrorMsg(''); }} className="w-full border p-2 rounded bg-gray-50 text-sm focus:ring-2 outline-none">Jemaat (Read-Only)Penatua (Akses Edit Rayon)Administrator (Akses Penuh)
{role === 'penatua' && Pilih Rayon<select value={selectedRayon} onChange={(e) => setSelectedRayon(e.target.value)} className="w-full border p-2 rounded bg-gray-50 text-sm focus:ring-2 outline-none">{Object.keys(penatuaMap).sort((a,b)=>parseInt(a)-parseInt(b)).map(r => Rayon {r} - {penatuaMap[r]})}}
{role !== 'jemaat' && Password<input type="password" required value={password} onChange={e=>setPassword(e.target.value)} className="w-full border p-2 rounded bg-gray-50 text-sm focus:ring-2 outline-none" />}
Masuk Dashboard
...
if (!formData?.noRayon) return Array.from({length:30},(,i)=>i+1);
...
return Array.from({length:100},(,i)=>i+1).filter(n => !used.includes(n));
...
const getTabHeaders = () => {
const r = (v) => R-${v}; const bld = (v) => {v}; const lp = (v) => isL(v)?'L':(isP(v)?'P':'-');
            ...
            <>
              ...
              <div className="p-0">
                <div className="hidden print:block mb-4 border-b-2 border-black pb-4">
                   ...
                   <p className="text-center font-medium text-sm">SILOFETOMONE</p>
                </div>
